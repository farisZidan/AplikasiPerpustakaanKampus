...
{{-- 1. BACKGROUND FIXED (Sama seperti halaman List) --}}
    ...
    <div class="w-full max-w-2xl bg-slate-900/60 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-white/10 bg-white/5 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold font-serif text-white">
                    {{ $is_update ? 'Edit Data Buku' : 'Tambah Buku Baru' }}
                </h2>
                <p class="text-slate-400 text-sm mt-1">Silakan lengkapi informasi buku di bawah ini.</p>
            </div>
            {{-- Icon Buku --}}
        ...
        <div class="p-8">
            ...
            <form action="{{ url('buku/save') }}" method="post" accept-charset="utf-8" class="space-y-6">
                ...
                {{-- Hidden Inputs --}}
                <input type="hidden" name="id" value="{{ $is_update ? ($query->ID_Buku ?? '') : '' }}" />
                ...
                <div class="group">
                    ...
                    <input type="text" 
                           name="Judul" 
                           {{-- Logic Value: Cek Old Input -> Cek Database (Edit) -> Kosong --}}
                           value="{{ old('Judul', $is_update ? ($query->Judul ?? '') : '') }}" 
                ...
                <div class="group">
                    ...
                    <input type="text" 
                           name="Pengarang" 
                           value="{{ old('Pengarang', $is_update ? ($query->Pengarang ?? '') : '') }}" 
                           class="w-full px-4 py-3 rounded-lg bg-slate-800/50 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                ...
                <div class="group">
                    ...
                    <div class="relative">
                        <select name="Kategori" 
                                class="w-full px-4 py-3 rounded-lg bg-slate-800/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all appearance-none cursor-pointer">
                            
                            <option value="" disabled {{ old('Kategori', $is_update ? ($query->Kategori ?? '') : '') ? '' : 'selected' }}>-- Pilih Kategori --</option>
                            
                            @foreach ($optkategori as $key => $value)
                                <option value="{{ $key }}" class="bg-slate-800 text-white"
                                    {{-- Logic Selected yang Benar --}}
                                    {{ old('Kategori', $is_update ? ($query->Kategori ?? '') : '') == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
